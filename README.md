...
Google Sheets 기능 목록으로부터 QA Test Case를 자동 생성하는 도구입니다. **Express API**가 파이프라인(Plan · Taxonomy · Generator · Evaluator 등)을 수행하고, **waterbean**은 파이프라인 실행과 SSE 모니터링용 React UI입니다.
...
├── waterbean/                  # React SPA — TC Harness (파이프라인 UI, :5174)
│   └── src/
│       ├── features/
...
| `waterbean` | 5174 | React 19 · Vite 8 · Tailwind 4 | Pipeline 실행 UI |
...
- **Evaluator**: TC 스키마, 필수 필드, 도메인 최소 세트, 커버리지, 중복 검사. repair 라운드 가능.
...
요청 본문에는 **`targetSheetName`(필수)**·`spreadsheetUrl`·Fallback·`skillId` 등이 포함되며, `domainMode`·`domainScope`·`implementation`은 받지 않습니다.
